...
contract ArtDecoNFT is ERC721URIStorage, Ownable {
    ...
    modifier _validateBid(uint256 _id, uint256 _price) {
        ...
        require (_price > 0);
        require (_msgSender() != ownerOf(_id));
        require (bidStatusOfToken[_id][_msgSender()] == false);
        _;
    }
    ...
    modifier _validateCancelBid(uint256 _id) {
        require (_exists(_id));
        require (listedMap[_id]);
        require (bidStatusOfToken[_id][_msgSender()]);
        _;
    }
    ...
    modifier _validateSell(uint256 _id, address _buyer) {
        ...
        require (bidStatusOfToken[_id][_buyer] == true);

        uint256 _bidPrice = getPriceOfBid(_id, _buyer);
        require (_bidPrice <= paymentToken.allowance(_buyer, address(this)));
        require (_bidPrice <= paymentToken.balanceOf(_buyer));
        _;
    }
    ...
    function cancelBid(uint256 _id) external _validateCancelBid(_id) {
        for (uint i = 0; i < bidArrayOfToken[_id].length; i ++) {
            if (bidArrayOfToken[_id][i].buyer == _msgSender()) {
                uint lastIndex = bidArrayOfToken[_id].length - 1;
                bidArrayOfToken[_id][i] = bidArrayOfToken[_id][lastIndex];
    ...
    function sell(uint256 _id, address _buyer) external _validateSell(_id, _buyer) {
        ...
        uint256 _commissionValue = _price / (10**2) * feePercent;
        uint256 _sellerValue = _price - _commissionValue;
